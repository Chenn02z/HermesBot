...
- amber note = spec, milestone, or doc tension
...
- **Title**: short, names the deepening, such as "Deepen finance brief evidence pipeline".
- **Badge row**: recommendation strength (`Strong` = emerald, `Worth exploring` = amber, `Speculative` = slate), plus the area (`workflow`, `skills`, `agents`, `finance`, `tests`, `docs`).
...
- **Spec/milestone/doc callout**: one line in an amber-tinted box when relevant.
...
<div class="rounded-lg border border-slate-200 bg-white p-4">
  <pre class="mermaid">
    flowchart LR
      A[Skill] --> B[Spec status checks]
      B --> C[Milestone handoff]
      C -.leak.-> D[Duplicated workflow rule]
...
through. Before: 6 thin layers each doing little. After: 1 thick band labelled
...
- "Finance brief rendering is shallow: callers assemble too much state."
- "Spec status logic leaks across the seam."
- "Deepen: one interface, one place to test."
- "Two adapters justify the seam: fixture-backed tests and future live providers."

**Wins bullets** name the gain in glossary terms: "locality: one module owns
rules", "leverage: one interface, many tests", "interface shrinks;
implementation absorbs call choreography". Do not write "cleaner code" unless
the sentence says what changed structurally.
